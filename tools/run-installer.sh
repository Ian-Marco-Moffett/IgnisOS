mkdir -p var/
mkdir -p var/log/
qemu-system-x86_64 --enable-kvm -cpu qemu64 -M pc -m 3G -hda meta/disk.img -cdrom IgnisInstaller.iso -boot d -monitor stdio -smp 4 -rtc base=localtime -audiodev pa,id=audio0 -machine pcspk-audiodev=audio0 -serial file:var/log/ignis.log
