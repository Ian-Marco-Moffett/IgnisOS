mkdir -p var/
mkdir -p var/log/
qemu-system-x86_64 --enable-kvm -cpu qemu64 -M q35 -m 3G -drive file=Ignis.iso -boot d -monitor stdio -smp 4 -rtc base=localtime -audiodev pa,id=audio0 -machine pcspk-audiodev=audio0 -serial file:var/log/ignis.log
...
# SERIAL TO STDIO DEBUG
# qemu-system-x86_64 --enable-kvm -cpu host -M q35 -m 3G -drive file=Ignis.iso -boot d -smp 4 -rtc base=localtime -audiodev pa,id=audio0 -machine pcspk-audiodev=audio0 -serial stdio --no-reboot --no-shutdown
